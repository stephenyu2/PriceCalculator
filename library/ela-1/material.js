...
function renderWorksheet(data, el) {
  el.innerHTML = `
    ${header(data, `${data.difficulty} Worksheet`)}
    ${stubNotice(data)}
    ${answerKeyBar()}
    <ul class="worksheet-problems">
      ${data.items.map((p, i) => problemItem(p, i + 1, false)).join('')}
    </ul>`;
}
...
function renderQuiz(data, el) {
  el.innerHTML = `
    ...
    ${answerKeyBar()}
    <ul class="worksheet-problems">
      ${data.items.map((p, i) => problemItem(p, i + 1, true)).join('')}
    </ul>`;
}

// ── Shared problem item ───────────────────────────────────────────────────────

function problemItem(p, num, showDifficulty) {
  const diffTag = showDifficulty && p.difficulty
