...
ELA rules: reading items must embed any needed passage in the item itself; difficulty scales by passage complexity and depth of analysis. No em dashes. No math. Output ONLY the JSON file (use the Write tool). Reply with one line: "blueprint written: ${t.standard}".`
}
...
Self-contained: embed any passage/sentence the item needs in its prompt. No em dashes. Grade-appropriate. The keyed answer must be the single best, unambiguous answer.${fb}
...
Record its errors. For any arithmetic in any item, compute it with code (node/python), never by eye.

Step 2 — INDEPENDENT RE-SOLVE: for each item, read only the prompt+stimulus, solve it yourself, THEN compare to the key. Check: your answer matches the key; for quizzes the keyed letter is the single best option and every distractor is genuinely wrong; solution.steps are correct, complete, no leaps; the item is aligned to ${t.standard} and grade-${t.grade} appropriate; the stimulus is self-contained. Bias toward rejection when ambiguous.
