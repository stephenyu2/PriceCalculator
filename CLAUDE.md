...
/parent-portal             → parent-portal.html (hub: new families, migrating customers, plan management, six enrollment perks, resources). Also hosts the "Register for a Diagnostic Test" wizard: an in-page modal that branches Academic (subject → grade K-12/College) vs SAT/ACT (SAT-or-ACT → ScoreSmart/Bluebook), then proctored-this-Sunday-10AM (dynamic date) yes/no; No → pick day+time + location; ends collecting student name + email and POSTs the `diagnostic-registration` Netlify form (which sends a confirmation via the submission-created function). The card notes "maximum of one registration per subject per week" as display copy only (not enforced, no login to track it). SCHEDULING SPLIT (added 2026-09-01): ScoreSmart needs a tutor to manually assign test credits before each test, so the ScoreSmart leg uses a "Weekend Diagnostic Testing" model to batch that manual work: an info screen (register-by-Thursday-8PM copy + cadence guidance: >1.5 months from test date → every 2 weeks, within 1.5 months → weekly), then proctored/self-schedule limited to Fri/Sat/Sun of the eligible weekend (Thursday-8PM cutoff rolls to next weekend once passed; computeWeekend() in the wizard JS), 6AM-8PM 30-min slots, no 24h minimum. Bluebook and Academic legs are unchanged: this-week-only, 24h minimum, 6AM-8PM 30-min slots (they need no credit assignment). Fully automating credit assignment was ruled out: ScoreSmart's API is a Premium white-label add-on for exporting reports, with no public docs and no confirmed "grant a credit" endpoint or roster import.
