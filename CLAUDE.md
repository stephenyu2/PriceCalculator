...
/parent-portal             → parent-portal.html (hub: new families, migrating customers, plan management, six enrollment perks, resources). Also hosts the "Register for a Diagnostic Test" wizard: an in-page modal that branches Academic (subject → grade K-12/College) vs SAT/ACT (SAT-or-ACT → ScoreSmart/Bluebook), then proctored-this-Sunday-10AM (dynamic date) yes/no; No → pick day+time + location; ends collecting student name + email and POSTs the `diagnostic-registration` Netlify form (which sends a confirmation via the submission-created function). The card notes "maximum of one registration per subject per week" as display copy only (not enforced, no login to track it). SCHEDULING SPLIT (added 2026-09-01): the whole SAT/ACT branch (BOTH ScoreSmart and Bluebook) uses a "Weekend Diagnostic Testing" model, originally driven by ScoreSmart needing a tutor to manually assign test credits before each test and batching that work into one window: an info screen (register-by-Thursday-8PM copy WITH the live Thursday cutoff date + Fri-Sun dates, plus cadence guidance: >1.5 months from test date → every 2 weeks, within 1.5 months → weekly), then proctored/self-schedule limited to Fri/Sat/Sun of the eligible weekend (Thursday-8PM cutoff rolls to next weekend once passed; computeWeekend() in the wizard JS, keyed on testType SAT/ACT via isWeekendMode()), 6AM-8PM 30-min slots, no 24h minimum. The parent-portal diagnostic card is titled "Weekend Diagnostic Test" and shows the same weekend window + cadence copy with live dates BEFORE the wizard opens (dt-card-cutoff/dt-card-range filled by the wizard IIFE). The Academic leg is unchanged: this-week-only, 24h minimum, 6AM-8PM 30-min slots. Fully automating ScoreSmart credit assignment was ruled out: its API is a Premium white-label add-on for exporting reports, with no public docs and no confirmed "grant a credit" endpoint or roster import.
